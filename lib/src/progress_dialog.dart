...
class ProgressDialog extends StatefulWidget {
  ...
  /// ```
  static void update(GlobalKey<ProgressDialogState> key, {required Config config}) {
    key.currentState?.update(config: config);
  }
  ...
  static void safeClose(GlobalKey<ProgressDialogState> key) {
    if (key.currentWidget != null && key.currentContext != null) {
      try {
        ...
        log("Error while trying to safely close progress dialog", error: e, stackTrace: stackTrace);
      }
    }
  }
}
...
class ProgressDialogState extends State<ProgressDialog> {
      ...
      child: AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Row(
              children: [
                _config.maxProgress! > 0 && _config.progress == _config.maxProgress
          ...
          child: Stack(
            ...
            children: [
              CircularProgressIndicator.adaptive(
                valueColor: AlwaysStoppedAnimation<Color>(_getEffectiveProgressValueColor()),
                backgroundColor: _config.progressBgColor ?? Theme.of(context).colorScheme.background,
                value: (_config.progressType == ProgressType.normal || _config.progress == 0 || _config.maxProgress == 0
                    ? null
                    : _config.progress! / _config.maxProgress!),
              ),
              if (_config.valuePosition == ValuePosition.inProgress) _createValueWidget(),
            ],
          ),
        );
  }
  ...
  Widget _createMessageWidget() {
    return DefaultTextStyle.merge(
      ...
      child: Text(
        _config.progress == _config.maxProgress! ? _config.completed?.message ?? _config.message! : _config.message!,
  ...
  Widget _createValueWidget() {
    return DefaultTextStyle.merge(
      style: TextStyle(
        fontSize: _config.valuePosition != ValuePosition.inProgress ? 15.0 : 13.0,
        fontWeight: _config.valuePosition != ValuePosition.inProgress ? FontWeight.normal : FontWeight.bold,
